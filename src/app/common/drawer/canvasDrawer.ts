...
const applyFillOptions = (state: CanvasDrawerState, fillOptions: any) => {
...
const circle = (state: CanvasDrawerState, centerPosition: Point2D, radius: number, lineOptions?: LineOptions, fillOptions?: any): Path2D => {
  const sector: CircularSector = { position: centerPosition, radius, arc: { start: 0, end: 2 * Math.PI } }
  const p = arc(state, sector, lineOptions)

  return p
}

const path = (state: CanvasDrawerState, vertices: Point2D[], lineOptions?: LineOptions, fillOptions?: any): Path2D => {
...
const rect = (state: CanvasDrawerState, _rect: Rect, lineOptions: LineOptions, fillOptions: any): Path2D => {
  if (lineOptions != null)
    applyLineOptions(state, lineOptions)
...
const isoscelesTriangle = (state: CanvasDrawerState, boundingRect: Rect, lineOptions: LineOptions, fillOptions: any): Path2D => {
  if (lineOptions != null)
    applyLineOptions(state, lineOptions)
...
export const createCanvasDrawer = (
  ...
  return {
    ...
    circle: (centerPosition, radius, lineOptions, fillOptions) => circle(state, centerPosition, radius, lineOptions, fillOptions),
    path: (vertices, lineOptions, fillOptions) => path(state, vertices, lineOptions, fillOptions),
    rect: (_rect, lineOptions, fillOptions) => rect(state, _rect, lineOptions, fillOptions),
    isoscelesTriangle: (boundingRect, lineOptions, fillOptions) => isoscelesTriangle(state, boundingRect, lineOptions, fillOptions),
